...
use earmark_store::{BatchWrite, CanonicalStore, GitCanonicalStore, StoredObject, StoredPayload, StoreError};
...
fn test_payload_integrity_verification() {
    ...
    let payload_file = fs::read_dir(&version_dir).unwrap()
        .filter_map(Result::ok)
        .map(|e| e.path())
        .find(|p| p.file_name().unwrap().to_str().unwrap().starts_with("payload."))
    ...
    assert!(success, "Failed to corrupt payload file after 5 seconds due to locking");
...
fn test_rollback_cleanup_intermediate_dirs() {
    ...
    let obj_dir = root.join(".earmark").join("canonical").join("objects").join(obj_id_str);
    assert!(!obj_dir.exists(), "Object directory should be cleaned up if empty during rollback");
}
...
fn test_write_serialization_locking() {
    ...
    assert!(elapsed >= std::time::Duration::from_millis(200), "Second lock acquisition should have been blocked");
}
