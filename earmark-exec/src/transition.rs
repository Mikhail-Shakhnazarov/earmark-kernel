...
use earmark_core::{
    AssignmentStatus, ChangeSetDraft, ChangeSetValidationResult, ChangeSetId, Kind, ObjectRef,
...
use crate::provider::{provider_record_from_failure, provider_record_from_response, ProviderMode, resolve_provider_profile};
...
use crate::helpers::{
    reject_duplicate_active_assignment, record_transition, store_work_packet, uuid_like,
...
impl<'a, S: CanonicalStore> ExecutionEngine<'a, S> {
    ...
    pub(crate) fn execute_transition<C: CompiledContextCompiler<S>>(
...

        {
            let change_set_id = ChangeSetId::new();
            let now_end = Utc::now();
            let (validation_result, standing_requests) =
                validate_transition_change_set(
                    store,
                    index,
                    system,
                    transition,
            ...
            if !validation_result.is_valid {
                ...
                assignment.blocked_reason = Some(format!(
                    ...
                    blocked_change_set_id.as_str(), failure_ref.id.as_str()
                ));
            ...
            for spec in handoff_specs {
                ...
                let stored_handoff = StoredObject::new(
                    ...
                    BTreeMap::from([(
                        "title".to_string(),
                        earmark_core::HeaderValue::String(format!("Handoff for {}", handoff.id.as_str())),
                    )]),
