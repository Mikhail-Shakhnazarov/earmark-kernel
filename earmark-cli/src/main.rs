...
use earmark_core::{
    ClassFilter, EpistemicStanding, HeaderValue, Kind, ObjectRef, Provenance, RelationFilter,
    Standing, StandingFilter, VersionRef,
};
use earmark_declarations::{
    activate_system_definition, load_class_definition, load_workflow_definition,
...
fn run(cli: Cli) -> Result<(), CliError> {
    let store = GitCanonicalStore::new(&cli.root);
    store.init_layout()?;
    let index = DerivedIndex::open(&cli.root)?;
    let mut provider_registry = ProviderRegistry::default();
    provider_registry.register(std::sync::Arc::new(earmark_exec::MockAdapter));
    provider_registry.register(std::sync::Arc::new(earmark_exec::GeminiAdapter::new(
    ...
    match cli.command {
        ...
        Commands::System(command) => match command.action {
            SystemAction::Register { manifest } => {
                ...
                emit(
                    ...
                    json!({
                        ...
                        "object_id": version_ref.id.0,
                        "version_id": version_ref.version_id.0,
                    }),
                );
            }
        ...
        Commands::Deposit(args) => {
            ...
            let payload = build_payload(&kind, &args)?;
            let mut headers = BTreeMap::new();
            if let Some(title) = &args.title {
                headers.insert("title".to_string(), HeaderValue::String(title.clone()));
            }
            ...
            emit(
                ...
                json!({
                    ...
                    "object_id": version_ref.id.0,
                    "version_id": version_ref.version_id.0,
                    "kind": object.envelope.kind.as_str(),
                    "class": object.envelope.class,
                    "title": object.envelope.title(),
                }),
            );
        }
        Commands::Query(args) => {
            ...
            emit(cli.json, serde_json::to_value(rows)?);
        }
        Commands::Review(args) => {
            ...
            emit(
                ...
                json!({
                    "ok": true,
                    "review_object_id": review.envelope.id.0,
                    "review_version_id": review.envelope.version_id.0,
                    "target_object_id": target_object.envelope.id.0,
        ...
        Commands::Workflow(command) => match command.action {
            WorkflowAction::Run(args) => {
                index.rebuild_from_store(&store)?;
                let workflow =
                    resolve_version_ref(&store, &args.workflow_id, args.version_id.as_deref())?;
                let system = resolve_system_version_ref(&index, &args.system_id)?;
                let inputs = args
                    .inputs
                    .iter()
        ...
        Commands::Assignment(command) => match command.action {
            AssignmentAction::Show { assignment_id } => {
                ...
                emit(cli.json, serde_json::to_value(assignment)?);
            }
            AssignmentAction::Explain { assignment_id } => {
                ...
                emit(
                    ...
                    json!({
                        ...
                        "kind": "assignment",
                        "id": assignment_id,
                        "summary": format!("assignment {} in status {}", assignment.id.0, format!("{:?}", assignment.status).to_lowercase()),
                        "artifact": assignment.clone(),
                        "related": {
                            "run_id": assignment.run_id.clone(),
                            "transition_id": assignment.transition_id.clone(),
                            "completion_change_set_id": assignment.completion_change_set_id.as_ref().map(|id| id.0.clone()),
                            "handoff_manifest_id": assignment.handoff_manifest_id.as_ref().map(|id| id.0.clone()),
                        },
            ...
            AssignmentAction::List { run_id, status } => {
                ...
                emit(cli.json, serde_json::to_value(assignments)?);
            }
        },
        Commands::ChangeSet(command) => match command.action {
            ChangeSetAction::Show { change_set_id } => {
                ...
                emit(cli.json, serde_json::to_value(change_set)?);
            }
            ChangeSetAction::Explain { change_set_id } => {
                ...
                emit(
                    ...
                    json!({
                        ...
                        "kind": "change_set",
                        "id": change_set_id,
                        "summary": format!("change set {} for transition {}", change_set.id.0, change_set.transition_id),
                        "artifact": change_set.clone(),
                        "related": {
                            "run_id": change_set.run_id.clone(),
                            "assignment_id": change_set.assignment_id.as_ref().map(|id| id.0.clone()),
                            "handoff_manifest_id": change_set.handoff_manifest_id.as_ref().map(|id| id.0.clone()),
            ...
            ChangeSetAction::List { run_id } => {
                ...
                emit(cli.json, serde_json::to_value(change_sets)?);
            }
        },
        Commands::Handoff(command) => match command.action {
            HandoffAction::Show { handoff_id } => {
                ...
                emit(cli.json, serde_json::to_value(handoff)?);
            }
            HandoffAction::Explain { handoff_id } => {
                ...
                emit(
                    ...
                    json!({
                        ...
                        "kind": "handoff",
                        "id": handoff_id,
                        "summary": format!("handoff {} from transition {}", handoff.id.0, handoff.from_transition_id),
                        "artifact": handoff.clone(),
                        "related": {
                            "run_id": handoff.run_id,
                            "source_change_set_id": handoff.source_change_set_id.0,
                            "source_assignment_id": handoff.source_assignment_id.clone().map(|id| id.0),
            ...
            HandoffAction::List { run_id } => {
                ...
                emit(cli.json, serde_json::to_value(handoffs)?);
            }
        },
        Commands::Failure(command) => match command.action {
            FailureAction::Show { failure_id } => {
                ...
                emit(cli.json, serde_json::to_value(failure)?);
            }
                ...
                emit(
                    ...
                    json!({
                        ...
                        "related": {
                            ...
                            "assignment_id": failure.assignment_id.0,
                            "failed_change_set_id": failure.failed_change_set_id.clone().map(|id| id.0),
        ...
        Commands::Context(command) => match command.action {
            ContextAction::Compile(args) => {
                ...
                let manifest = runtime_surface.compile_connected_context(
                    args.roots.into_iter().map(earmark_core::ObjectId).collect(),
                    args.depth,
                    Some(RelationFilter {
                ...
                emit(cli.json, serde_json::to_value(manifest)?);
            }
        },
        Commands::Audit(command) => match command.action {
            AuditAction::Failures {
                ...
                emit(cli.json, json!({ "ok": true, "failures": failures }));
            }
...
fn build_payload(kind: &Kind, args: &DepositArgs) -> Result<StoredPayload, CliError> {
    let text = if let Some(path) = &args.payload_file {
        fs::read_to_string(path)?
    } else if let Some(json) = &args.json_payload {
        json.clone()
    } else {
    ...
    let payload = match kind {
        Kind::Instruction => StoredPayload::new(PayloadEncoding::Markdown, text.into_bytes()),
        Kind::Workflow
        | Kind::Policy
        | Kind::CompiledContextTemplate
        | Kind::ProviderProfile
        | Kind::SystemDefinition => StoredPayload::new(PayloadEncoding::Yaml, text.into_bytes()),
        Kind::Relation | Kind::Event | Kind::WorkPacket | Kind::RunRecord => {
            StoredPayload::new(PayloadEncoding::Json, text.into_bytes())
        }
        _ => {
            if args.json_payload.is_some() {
                StoredPayload::new(PayloadEncoding::Json, text.into_bytes())
            } else {
                StoredPayload::new(PayloadEncoding::Markdown, text.into_bytes())
            }
        }
    };
    Ok(payload)
    ...
    match kind {
        ...
        DeclarationKind::Instruction => {
            ...
            Ok(json!({
                ...
                "body_preview": declaration.body.0.lines().take(3).collect::<Vec<_>>().join("\n"),
            }))
        }
        DeclarationKind::StandingPolicy => {
            let declaration = load_standing_policy(path)?;
            ...
            Ok(json!({
                ...
                "operations": declaration.operations.iter().map(|operation| json!({
                    ...
                    "standing_implications": operation.policy.as_ref().map(|policy| vec![format!(
                        "policy-bound: {}@{}",
                        policy.id.0, policy.version_id.0
                    )]).unwrap_or_default(),
                })).collect::<Vec<_>>(),
...
fn register_declaration_file<S: CanonicalStore>(
    store: &S,
    kind: DeclarationKind,
    path: &PathBuf,
) -> Result<VersionRef, CliError> {
    let (stored_kind, name, payload, headers, explicit_id) = match kind {
        DeclarationKind::Class => {
    ...
    if let Some(id) = explicit_id {
        object.envelope.id = earmark_core::ObjectId(id);
    }
...
fn resolve_version_ref<S: CanonicalStore>(
    ...
    if let Some(version_id) = version_id {
        return Ok(VersionRef::new(
            earmark_core::ObjectId(object_id.to_string()),
            earmark_core::VersionId(version_id.to_string()),
        ));
    }
    store
        .read_head_ref(&earmark_core::ObjectId(object_id.to_string()))?
        .ok_or_else(|| CliError::NotFound(format!("object head not found: {}", object_id)))
}
...
fn resolve_object_ref<S: CanonicalStore>(
    ...
    let head = store
        .read_head(&earmark_core::ObjectId(object_id.to_string()))?
        .ok_or_else(|| CliError::NotFound(format!("object not found: {}", object_id)))?;
    Ok(head.object_ref())
}
...
fn load_run_record_by_id<S: CanonicalStore>(
    ...
    Err(CliError::NotFound(format!("run not found: {}", run_id)))
}
...
fn list_failure_objects<S: CanonicalStore>(
    ...
    for object in store.scan_objects()? {
        ...
        let failure: earmark_core::TransformationFailure =
            serde_json::from_slice(&object.payload.bytes)?;
        failures.push((object.envelope.id.0.clone(), failure));
    }
    Ok(failures)
}
...
fn load_current_assignment_by_id<S: CanonicalStore>(
    ...
    for object in store.scan_objects()? {
        ...
        let assignment: earmark_core::TransitionAssignment = serde_json::from_slice(&object.payload.bytes)?;
        if assignment.id.0 != assignment_id {
            continue;
        }
        if let Some(head_ref) = store.read_head_ref(&object.envelope.id)? {
            if head_ref.version_id == object.envelope.version_id {
    ...
    Err(CliError::NotFound(format!("assignment not found: {}", assignment_id)))
}
...
fn load_change_set_by_id<S: CanonicalStore>(
    ...
    for object in store.scan_objects()? {
        ...
        let change_set: earmark_core::ChangeSet = serde_json::from_slice(&object.payload.bytes)?;
        if change_set.id.0 == change_set_id {
            return Ok(change_set);
        }
    }
    Err(CliError::NotFound(format!("change set not found: {}", change_set_id)))
}
...
fn load_failure_by_id<S: CanonicalStore>(
    ...
    for object in store.scan_objects()? {
        ...
        if object.envelope.id.0 == failure_id {
            let failure: earmark_core::TransformationFailure =
                serde_json::from_slice(&object.payload.bytes)?;
            return Ok(failure);
        }
...
fn parse_epistemic(value: &str) -> Result<EpistemicStanding, CliError> {
    match value {
...
fn resolve_system_version_ref(
    ...
    let found = index
        ...
        .ok_or_else(|| CliError::NotFound(format!("system definition not found: {}", system_id)))?;
    Ok(VersionRef::new(
...
fn mirror_surface(store: &GitCanonicalStore, object: &StoredObject) -> Result<(), CliError> {
    ...
    let path = dir.join(format!("{}.{}", object.envelope.id.0, ext));
...
fn next_commands_for_assignment(assignment: &earmark_core::TransitionAssignment) -> Vec<String> {
    ...
    if let Some(hid) = &assignment.handoff_manifest_id {
        commands.push(format!("em handoff explain {}", hid.0));
    }
    if let Some(did) = &assignment.completion_change_set_id {
        commands.push(format!("em change-set explain {}", did.0));
    }
...
fn next_commands_for_change_set(change_set: &earmark_core::ChangeSet) -> Vec<String> {
    ...
    if let Some(aid) = &change_set.assignment_id {
        commands.push(format!("em assignment explain {}", aid.0));
    }
...
fn next_commands_for_failure(failure: &earmark_core::TransformationFailure) -> Vec<String> {
    ...
    if let Some(delta_id) = &failure.failed_change_set_id {
        commands.push(format!("em change-set explain {}", delta_id.0));
    }
    commands.push(format!("em assignment explain {}", failure.assignment_id.0));
    commands.push(format!("em run timeline {}", failure.run_id));
    commands
}

fn emit(as_json: bool, value: serde_json::Value) {
    if as_json {
        let envelope = json!({
