...
use earmark_core::{Kind, ObjectId, VersionId, Standing, Provenance, StandingTransitionRequest, StandingRequestStatus};
...
fn inject_standing_request(root: &std::path::Path, status: StandingRequestStatus) -> (ObjectId, VersionId) {
...
fn standing_request_apply_outputs_machine_readable_json() {
    ...
    let target_obj = StoredObject::new(
        ...
        Provenance::direct_input("test"),
        std::collections::BTreeMap::new(),
        StoredPayload { format: earmark_store::PayloadEncoding::Json, bytes: b"{}".to_vec() },
        vec![],
    );
    ...
    let req_obj = StoredObject::new(
        ...
        Provenance::direct_input("test"),
        std::collections::BTreeMap::new(),
        StoredPayload { format: earmark_store::PayloadEncoding::Json, bytes: payload_bytes },
        vec![],
    );
