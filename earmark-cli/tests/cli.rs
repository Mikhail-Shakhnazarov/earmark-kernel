...
#[test]
fn declare_new_without_path_uses_workspace_declarations_dir() {
    let dir = tempdir().unwrap();
    Command::cargo_bin("earmark-cli")
        .unwrap()
        .arg("--root")
    ...
    let parsed: Value = serde_json::from_slice(&output).unwrap();
    assert_eq!(parsed["ok"], true);
    let path = dir.path().join(".earmark/declarations/classes/bug.yaml");
    assert!(path.exists());
    assert_eq!(parsed["data"]["path"], path.display().to_string());
}

#[test]
fn declare_new_infers_instruction_and_workflow_contracts() {
    let dir = tempdir().unwrap();
    Command::cargo_bin("earmark-cli")
        .unwrap()
        .arg("--root")
    ...
    assert!(workflow.contains("id: compile_inputs"));
    assert!(workflow.contains("id: transform_output"));
}

#[test]
fn declare_new_class_template_is_registerable() {
    let dir = tempdir().unwrap();
    Command::cargo_bin("earmark-cli")
        .unwrap()
        .arg("--root")
    ...
    let class_path = dir.path().join(".earmark/declarations/classes/bug.yaml");
    Command::cargo_bin("earmark-cli")
        .unwrap()
        .arg("--root")
        .arg(dir.path())
...
#[test]
fn declare_new_instruction_template_is_valid() {
    let dir = tempdir().unwrap();
    Command::cargo_bin("earmark-cli")
        .unwrap()
        .arg("--root")
...
#[test]
fn declare_new_system_template_registers_for_single_stage_flow() {
    let dir = tempdir().unwrap();
    Command::cargo_bin("earmark-cli")
        .unwrap()
        .arg("--root")
    ...
    for (kind, name) in [
        ("class", "bug"),
        ("class", "friction"),
        ("instruction", "bug_to_friction"),
        ("compiled-context", "bug_to_friction"),
        ("workflow", "bug_to_friction"),
        ("system", "bug_to_friction"),
    ] {
        Command::cargo_bin("earmark-cli")
            .unwrap()
            .arg("--root")
    ...
    let system_path = dir
        .path()
        .join(".earmark/declarations/systems/bug_to_friction.yaml");
    Command::cargo_bin("earmark-cli")
        .unwrap()
        .arg("--root")
        .arg(dir.path())
