...
fn test_quickstart_smoke_path() {
    ...
    let deposit_output = Command::cargo_bin("earmark-cli").unwrap()
        ...
        .arg("--title").arg("Smoke Test Note")
        .arg("--body").arg("This is a smoke test.")
        .assert()
        .success()
        .get_output()
        .stdout.clone();
    ...
    let object_id = parsed["data"]["object_id"].as_str().expect("object_id missing");

    // 5. em query
    let query_output = Command::cargo_bin("earmark-cli").unwrap()
        .arg("--root").arg(root)
        .arg("--json")
        .arg("query")
        .arg("--class").arg("source_note")
    ...
    let items = parsed_query["data"].as_array().expect("query data not an array");
